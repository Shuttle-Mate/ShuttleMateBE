    ...
    public class SupportRequestController : ControllerBase
    {
        ...
        [FromQuery] int pageSize = 10)
        {
            return Ok(new BaseResponseModel<BasePaginatedList<ResponseSupportRequestModel>>(
                ...
                data: await _supportRequestService.GetAllAsync(category, status, search, userId, sortAsc, page, pageSize)));
        }
        ...
        public async Task<IActionResult> GetAllResponsesOfSupportRequest(Guid supportRequestId)
        {
            return Ok(new BaseResponseModel<IEnumerable<ResponseResponseSupportModel>>(
                statusCode: StatusCodes.Status200OK,
                code: ResponseCodeConstants.SUCCESS,
                data: await _supportRequestService.GetAllResponsesAsync(supportRequestId)));
        }
        ...
        /// </summary>
        //[Authorize(Roles = "Student", "Parent")]
        [HttpPatch("{supportRequestId}")]
        public async Task<IActionResult> UpdateSupportRequestStatus(Guid supportRequestId, UpdateSupportRequestModel model)
        {
            await _supportRequestService.UpdateStatusAsync(supportRequestId, model);
            return Ok(new BaseResponseModel<string?>(
               statusCode: StatusCodes.Status200OK,
               code: ResponseCodeConstants.SUCCESS,
